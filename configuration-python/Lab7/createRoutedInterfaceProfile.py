...
def input_optional_args(*arg):
    args = {}
    args['mtu'] = get_optional_input('DSCP (default: inherit): ', [], num_accept=True)
    args['targetDscp'] = get_optional_input('DSCP (default: 0): ', [], num_accept=True)
    return args


def create_interface_profile(modir, tenant_name, routed_outside_name, node_profile_name, interface_name, leaf_id, eth_num, ip_address, **args):
...
if __name__ == '__main__':
    ...
    create_interface_profile(modir, tenant_name, routed_outside_name, node_profile_name, interface_name, leaf_id, eth_num, ip_address, args_from_CLI=optional_args)
