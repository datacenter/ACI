from cobra.model.mgmt import Grp, InBZone, RsAddrInst, RsInbEpg, NodeGrp, RsGrp
...
class CreateNodeManagementAddress(CreateMo):
    ...
    def delete_mo(self):
        self.check_if_mo_exist('uni/infra/funcprof/grp-', self.policy_name, Grp, description='Managed Node Connectivity Group')
        self.mo.delete()
        self.commit_change()
        self.check_if_mo_exist('uni/infra/mgmtnodegrp-', self.policy_name, NodeGrp, description='The managed node group')
        self.mo.delete()
        self.commit_change()
        self.check_if_mo_exist('uni/tn-'+self.tenant+'/addrinst-', self.policy_name+'inbaddr', AddrInst, description='')
        self.mo.delete()
        self.commit_change()
